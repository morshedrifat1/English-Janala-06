...
const displayLesson = (lessonS) => {
    ...
    lessonS.forEach((lesson) => {
        ...
        createLessonDiv.innerHTML = `
            <button onclick="loadLessonContent(${lesson.level_no})" class="cursor-pointer border border-primary hover:bg-primary text-primary font-primary group hover:text-white text-sm font-semibold w-full px-2 py-[12px] rounded-md"><i class="fa-solid fa-book-open text-base mr-[6px] text-primary group-hover:text-white"></i>${lesson.lessonName}</button>
        `;
...
const lessonContainerDisplay = (lessonsData) => {
    document.getElementById("selectLesson").classList.add('hidden');
    const lessonContentContainer = document.getElementById("lessonContent-container");
        ...
        createLessonDiv.innerHTML = `
          ...
          <div class=" p-5 border border-gray-200 rounded-xl hover:bg-[#f1f8ff] hover:cursor-pointer group">
            ...
            <div class="flex justify-between pt-10">
              <button id="wordDetails" onclick="showLessonDetails(${lesson.id})" class="bg-[#e8f4ff] p-3 rounded-xl group-hover:bg-white hover:cursor-pointer border border-gray-200"><i class="fa-solid fa-circle-info text-xl text-[#374957]"></i></button>
...
const loadLessonDetails = (lessonDetail) => {
    ...
    const tyz = document.getElementById("ty");
    console.log(tyz);

      detailPopup.innerHTML = `
        <h1 class="font-primary font-semibold text-xl md:text-3xl text-black">${lessonDetail.word} (<i class="fa-solid fa-microphone-lines"></i><span> :${lessonDetail.pronunciation}</span>)</h1>
              <div class="space-y-3">
                <h3 class="font-primary font-semibold text-lg md:text-xl text-black">Meaning</h3>
                <p class="font-second text-xl font-medium text-black">${lessonDetail.meaning == null ? 'অর্থ পাওয়া যায়নি' : lessonDetail.meaning}</p>
              </div>
              <div class="space-y-3">
                <h3 class="font-primary font-semibold text:lg md:text-xl text-black">Example</h3>
                <p class="font-primary text-lg md:text-xl text-black">${lessonDetail.sentence}</p>
              </div>
              <div class="space-y-3">
                ...
                <button class="bg-[rgb(237,247,255)] border border-[#D7E4EF] font-primary text-base text-black rounded-lg px-4 py-2">${lessonDetail.synonyms[0]}</button>
                  <button class="bg-[#edf7ff] border border-[#D7E4EF] font-primary text-base text-black rounded-lg px-4 py-2">${lessonDetail.synonyms[1]}</button>
                  <button class="bg-[#edf7ff] border border-[#D7E4EF] font-primary text-base text-black rounded-lg px-4 py-2">${lessonDetail.synonyms[2]}</button>
                ` : ""}
